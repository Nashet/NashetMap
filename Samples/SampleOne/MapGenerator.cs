...
namespace Nashet.Map.Examples
{
	public class MapGenerator : MonoBehaviour
	{
		...
		private static int riverCrossingCost = 40;
		...
		private static void SetPatchFinding(Dictionary<int, Dictionary<int, MeshStructure>> meshes)
		{
			...
			foreach (var province in Province.AllProvinces)
			{
				foreach (var border in province.Value.neughbors)
				{
					...
					var path = new Path(border.Province.Node.autoGeneratedID, province.Value.Node.autoGeneratedID)
					{
						cost = (int)Vector3.Distance(border.Province.Position, province.Value.Position)
					};

					if (border.IsRiverBorder)
					{
						path.cost = path.cost + riverCrossingCost;
					}
